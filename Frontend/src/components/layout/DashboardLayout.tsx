...
import GateLogo from '../common/GateLogo';
...
const DashboardLayout = () => {
    ...
    const linkClass = ({ isActive }: { isActive: boolean }) =>
        `flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-175 select-none ${isActive
    ...
    return (
        <div className="min-h-screen flex bg-[#F6F6F6] dark:bg-[#000000]">
            ...
            {/* Sidebar */}
            <aside
                className={`fixed lg:static inset-y-0 left-0 z-50 w-[240px] bg-white dark:bg-[#141414] border-r border-[#E2E2E2] dark:border-[#242424] flex flex-col transform transition-transform duration-225 ease-[cubic-bezier(0.25,0.46,0.45,0.94)] lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'
                    }`}
            >
                {/* Logo area */}
                ...
                {/* User + logout */}
                <div className="px-3 pb-4 pt-3 border-t border-[#E2E2E2] dark:border-[#242424] flex-shrink-0 cursor-pointer" onClick={() => navigate(getProfilePath(user?.role as Role))}>
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <header className="sticky top-0 z-30 bg-white/90 dark:bg-[#000000]/90 backdrop-blur-xl border-b border-[#E2E2E2] dark:border-[#242424] flex items-center px-4 lg:px-6 justify-between" style={{ paddingTop: 'env(safe-area-inset-top)', minHeight: 'calc(4rem + env(safe-area-inset-top))' }}>
                    ...
                    <button
                        onClick={() => navigate(getProfilePath(user?.role as Role))}
                        className="p-2 rounded-full hover:bg-[#F6F6F6] dark:hover:bg-[#1C1C1C] text-[#545454] dark:text-[#9E9E9E] transition-colors"
                        aria-label="Profile"
                    >
...
function getProfilePath(role: Role) {
    const map: Record<string, string> = {
        [Role.SUPER_ADMIN]: '/super-admin/profile',
        [Role.ADMIN]: '/admin/profile',
        [Role.RESIDENT]: '/resident/profile',
